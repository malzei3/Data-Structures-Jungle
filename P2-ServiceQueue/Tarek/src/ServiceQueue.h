...
class ServiceQueue {
  ...
  private:
	 ...
	 std::vector<int> buzzer_bucket;
		 ...
		 node(int _id = 0, node* _prev=nullptr, node* _next=nullptr){
			 next = _next;
			 prev = _prev;
			 id  = _id;
		 };
  ...
  public:
	...
	/**
	 * Constructor
	 * Description: intializes an empty gservice queue.
	 * 
	 ...
	 *	IDs that have beeclear();n used during the lifetime of the
