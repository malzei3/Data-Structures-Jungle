...
class graph {
      ...
      while(!q.empty()) {
        ...
        for(edge &e : vertices[u].outgoing) {
          ...
          if(report[v].state == UNDISCOVERED) {
            ...
            // only update if still shortest path
  ...
  public:
    ...
    bool dag_critical_paths(vector<vertex_label> & rpt) {
      ...
      while(src < vertices.size()) {
        if (vertices[src].incoming.size() == 0) {
          rpt[src].dist = 0;
          rpt[src].pred = src;
        }
        src++;
      ...
      //while (vertices[src].incoming.size() != 0) {
      //  src++;  // get the src ID
      //}
    ...
    bool dag_num_paths(vector<vertex_label> & rpt) {
      ...
      for (int i = 0; i < toppo.size(); i++){
        u = toppo[i];
        if(vertices[u].incoming.size() == 0) rpt[u].pred = 1;

        for(edge &e : vertices[u].incoming){
          int v = e.vertex_id;
          rpt[u].pred = rpt[v].pred + rpt[u].pred;
        }
      }

      for (int i = toppo.size() - 1; i >= 0; i--){
        u = toppo[i];
        if(vertices[u].outgoing.size() == 0) rpt[u].npaths = 1;

        for(edge &e : vertices[u].outgoing){
          int v = e.vertex_id;
          rpt[u].npaths = rpt[v].npaths + rpt[u].npaths;
        }
      }
      ...
      for (int i = 0; i < order.size(); i++) {
        ...
        if (vertices[u].incoming.size() > 0) {
          for (int j = 0; j < vertices[u].incoming.size(); j++) {
            if (!discovered[vertices[u].incoming[j].vertex_id]) return false;
          }
        }
        discovered[u] = true; 
      }
     ...
     *   20 lines of code.
     */
    void _enum_paths(int target, vector<string> &paths, string seq) {
      int v;
      
      if (seq != "") seq = id2name(target) + " " + seq;
      else seq = id2name(target);
