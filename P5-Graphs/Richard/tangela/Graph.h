...
class graph {
        ...
        for(edge &e : vertices[u].outgoing) {
          ...
          if(report[v].state == UNDISCOVERED) {
            ...
            if(report[v].dist == report[u].dist + 1){
                report[v].npaths = report[v].npaths + s;
  ...
  public:
    ...
    for (int i = topoGraph.size() - 1; i >= 0; i--){
      u = topoGraph[i];
    ...
    bool valid_topo_order(const vector<int> & order) {
      if(has_cycle())
        return false;
    vector<bool> vistedArray = vector<bool>(num_nodes()); 
    int size = vistedArray.size();
    ...
    for (int i = 0; i < order.size(); i++){
      ...
      if (size > 0) {
        ...
        for (int j = 0; j < size; j++){
          if (vistedArray[vertices[u].incoming[j].vertex_id] == false) {
            return false;
          }
        }
      }
    ...
    bool enum_paths(int target, vector<string> &paths) {
     ...
     enum_paths_helper(target, paths, "");
      return true;
    }

	private:
	void enum_paths_helper(int i, vector<string> &paths, string strPath){
	 if(strPath == ""){
          strPath = id2name(i);
    }
    else{
        strPath = id2name(i) + " " + strPath;
    }         
    if (vertices[i].incoming.size() == 0){
    ...
    for (edge &e : vertices[i].incoming){
      enum_paths_helper(e.vertex_id, paths, strPath);
    }
  }
