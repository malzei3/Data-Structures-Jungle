...
class graph {
        ...
        for(edge &e : vertices[u].outgoing) {
          ...
          if(report[v].state == UNDISCOVERED) {
            ...
            q.push(v);
			  ...
			  if ((report[v].npaths != 0) && (report[v].dist-1 == report[u].dist)) {
				  report[v].npaths += report[u].npaths;
			  }
		  }
        }
  ...
  public:
    ...
    bool extract_path(const vector<vertex_label> & rpt, 
      ...
      // Check if dest vertex is unreachable
	  if (rpt[dest].pred == -1) 
	  {
		  path.clear();
		  return false;
	  }
	  // At this point, the dest vertex can be reached
	  _extract_path(rpt, path, dest);
      return true;  // placeholder
    }

  private:
	void _extract_path(const vector<vertex_label> &rpt, vector<int> &path, int v) 
	{
		if (rpt[v].pred != v) 
		{
			_extract_path(rpt, path, rpt[v].pred);
		}
		path.push_back(v);
	}
  ...
  public:
    ...
    bool dag_critical_paths(vector<vertex_label> & rpt) {
	  ...
	  for (int i = 0; i < num_nodes(); i++) 
	  {
		  ...
		  {
			  rpt.push_back(vertex_label(0, i, '?'));
			  q.push(i);
		  }
		  else 
		  {
			  rpt.push_back(vertex_label(-1, i, '?'));
		  }
	  }
...
bool dag_num_paths(vector<vertex_label> & rpt) {
  if(has_cycle())
	return false;
  // your code here...
  // Populate npaths for input nodes. npaths must be 0 since there are no io-paths passing
  // through input nodes. The same can be said for output nodes. Only intermediate nodes will
  // have npaths > 0.
  rpt.clear();
  std::vector<vertex_label> o_rpt;
  std::vector<vertex_label> i_rpt;
  _num_o_paths(o_rpt);
  _num_i_paths(i_rpt);
  for (int i = 0; i < num_nodes(); i++) 
  {
	  rpt.push_back(vertex_label(-1, -1, '?'));
	  if (vertices[i].incoming.size() == 0) 
	  {
		  rpt[i].npaths = o_rpt[i].npaths;
	  }
	  else if (vertices[i].outgoing.size() == 0) 
	  {
		  rpt[i].npaths = i_rpt[i].npaths;
	  }
	  else 
	  {
		  rpt[i].npaths = o_rpt[i].npaths * i_rpt[i].npaths;
	  }
  }
...
void _num_i_paths(vector<vertex_label> &rpt) 
{
  ...
  for (int i = 0; i < num_nodes(); i++) 
  {
	  rpt.push_back(vertex_label(-1, -1, 'u'));
	  num_in.push_back(vertices[i].incoming.size());
	  if (vertices[i].incoming.size() == 0) 
	  {
		  rpt[i].npaths = 1;
		  q.push(i);
...
// Count the number of paths to the output vertices from v_id.
void _num_o_paths(vector<vertex_label> &rpt) 
{
	int u, v;
	std::queue<int> q;
	std::vector<int> num_out;
    rpt.clear();
	for (int i = 0; i < num_nodes(); i++) 
	{
		rpt.push_back(vertex_label(-1, -1, 'u'));
		num_out.push_back(vertices[i].outgoing.size());
...
bool valid_topo_order(const vector<int> & order) {
  ...
  // Keep track if the node have been seen
...
void _enum_paths(int target, vector<string> &paths, vector<int> seq) 
{
	int v;
	...
	if (vertices[target].incoming.size() == 0) 
	{
		...
		paths.push_back(id_vec2string(seq));
		return;
	}
	for (edge &e : vertices[target].incoming) 
	{
		v = e.vertex_id;
		_enum_paths(v, paths, seq);
	}
	return;
}
