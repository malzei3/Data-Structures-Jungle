 ...
 */
  ...
  public:
    ...
    bool _get_ith(bst_node *r, int i, T &x) {
      ...
      if (r->leftSize < i) {
        return _get_ith(r->right, i - r->leftSize - 1, x);
      }
  ...
  public:
    ...
    int position_of(const T & x) {
      ...
      T temp;
      bool buffer = min(temp);
      if (x == temp) return 1;
      if (x == max()) return root->leftSize + root->rightSize + 1;
      return num_leq(x);  
    }
     ...
     * Runtime:  O(h) where h is the tree height
     */
    void _num_geq(bst_node *r, int &sum, const T &x) {
      if (r == nullptr) return;
      //if (r->val == x) {
        //sum++;
        //return _num_geq(r->right, sum, x);
      //}
      if (r->val >= x) {
        sum = sum + r->rightSize + 1;
        return _num_geq(r->left, sum, x);
      }
      return _num_geq(r->right, sum, x);
    }
  ...
  public:
     ...
     *
     **/
    void _num_leq(bst_node *r, int &sum, const T &x) {
      if (r == nullptr) return;
      //if (r->val == x) {
        //sum++;
        //return _num_leq(r->left, sum, x);
      //}
      if (r->val <= x) {
        sum = sum + r->leftSize + 1;
        return _num_leq(r->right, sum, x);
      }
      return _num_leq(r->left, sum, x);
    }
  ...
  public:
    ...
    //int _num_range(bst_node *r, const T & min, const T & max) {
      //if (r == nullptr) return 0;
      //if (r->val == min && r->val == max) return 1;
      //else if (r->val < min) return _num_range(r->right, min, max);
      //else if (r->val > max) return _num_range(r->left, min, max);
      //else if (r->val >= min && r->val <= max) return 1 + _num_range(r->left, min, max) + _num_range(r->right, min, max);
    //}

    int num_range(const T & min, const T & max) {
      if (root == nullptr) return 0;
      return (num_leq(max) + num_geq(min)) - (root->leftSize + root->rightSize + 1);
  ...
  public:
     ...
     *
     */
    void _extract_range(std::vector<T> * vect, bst_node *r, const T & min, const T & max) {
      if (r == nullptr) return;
      if (r->val == min && r->val == max) vect->push_back(r->val);
      else if (r->val < min) _extract_range(vect, r->right, min, max);
      else if (r->val > max) _extract_range(vect, r->left, min, max);
      else if (r->val >= min && r->val <= max) {
        _extract_range(vect, r->left, min, max); 
        vect->push_back(r->val);
        _extract_range(vect, r->right, min, max);
      }
      return;
    }
    ...
    std::vector<T> * extract_range(const T & min, const T & max) {
      ...
      _extract_range(vect, root, min, max);
