   ...
   */
    ...
    */
    bool is_pareto() const{
	 int d = 2; // the second node in the linked list
      struct Node* a; //pointer to the node
      struct Node* lst;   // node pointer
      a = front;  //a is now pointing to the head of the linked list
      lst = front;  // lst is now pointing to the head of the linked list
      while (lst != nullptr && lst->next != nullptr){ //making sure its not an empty linked list and thats also how my loop terminates
        for(int c = 1;  c != d; c++){
          if ((a->price == lst->next->price && a->time == lst->next->time)
           || (a->price < lst->next->price && a->time < lst->next->time)
           || (a->price < lst->next->price && a->time == lst->next->time)
           || (a->price == lst->next->price && a->time < lst->next->time)){
             return false;
          }
           a = a->next; // traversing in the linked list with a
           
        }
        lst = lst->next; // 
        d++;
        a = front; //re-intialzing back to the head node in the linked list
      }
	  return true;
    ...
    */
    bool is_pareto_sorted() const{
	 struct Node* lst;
      ...
      while (lst!= nullptr && lst->next != nullptr){
        ...
        if ((lst->next->price < lst->price) || (lst->next->time > lst->time )){
          return false;
        }
        else{
          lst = lst->next;
    ...
    bool insert_sorted(double price, double time) {
       ...
       Node* temp = front;
       Node* tempNode = new Node(price,time, nullptr);
          //if the list is empty add node
          if(front == nullptr){
            front = tempNode;
            _size++;
            return true;
          }
          //If there is only one node there
          if(front->next == nullptr){
            if(front->price < tempNode->price){
              front->next = tempNode;
              _size++;
              return true;
          ...
          //more than one node!
          while (temp->next != nullptr){
            if (tempNode->price < temp->price ){
              tempNode->next = temp;
              front = tempNode; 
              _size++;
              return true;
            }
            if((tempNode->price > temp->price) && (tempNode->price == temp->next->price)){
              if(tempNode->time < temp->next->time){
            ...
            if(temp->price == temp->next->price){
              if(tempNode->price == temp->price){
                if((tempNode->time > temp->time) && (tempNode->time < temp->next->time)){
                  tempNode->next = temp->next;
                  temp->next = tempNode;
                  _size++;
                  return true;
    ...
    //     if(is_dominated){
    //       size++;
    //       // checking if insert vector is dominated by anyone in the list
    //       if((temp->price < tempNode->price) && (temp->time < tempNode->time)){
    //         return true;
    //       }
    //       if ((temp->price == tempNode->price) && (temp->time < tempNode->time)){
    //         return true;
    ...
    //       // have to maintain the sorted and pareto
    //       // if ((temp->price == tempNode->price) || (temp->time == tempNode->time)){
    //       //   return true;
    //       // }
    //     }
    ...
    //     //   size++;
    //     //   if (size != _size){
    //     //     if (tempNode->price < temp->price) && (tempNode->time < temp->time){
    //     //       //delete a node and deallocate the memory
    //     //       size = size - 1;
    //     //       _size = _size - 1;
    //     //     }  
    ...
    //     }
    //     temp = temp->next;//this is my while loop
    //   }
    // temp->next = tempNode;
    // return true;
    // }
    ...
    // }
      ...
      while (p != nullptr) {
        if ((insert->price >= p->price) &&(insert->time > p->time)) {
              //std::cout<<"\nOption sucks!"<<std::endl;
              return true;
        }
   ...
   */
    TravelOptions * union_pareto_sorted(const TravelOptions &other)const{
 	if(!is_pareto_sorted() || !other.is_pareto_sorted())
	  return nullptr;
    //TravelOption* otherTravel = new TravelOption();
    //Node* temp = front;
