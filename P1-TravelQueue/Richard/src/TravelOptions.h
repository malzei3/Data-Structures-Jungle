...
class TravelOptions{
   ...
   */
    ...
    */          
    static Relationship compare(double priceA, double timeA, 
      ...
      // compare for if equal
      if (priceA == priceB && timeA == timeB) return equal;

      // compare for a is better than b
      else if (priceA <= priceB && timeA <= timeB) return better;

      // compare for a is worse than b
      else if (priceA >= priceB && timeA >= timeB) return worse;
   ...
   */
    ...
    */
    bool is_sorted()const{
      ...
      // empty or only 1 node in list
      if (p == nullptr || p->next == nullptr) return true;

      while(p->next != nullptr) {

        // if current is bigger than the next
        if (p->price > p->next->price) return false;

        // if equal, then base on time. if current time is bigger than next, return false
        if ((p->price == p->next->price) && 
            (p->time > p->next->time)) return false;
    ...
    bool is_pareto() const{
      ...
      Node *p = front;
	    ...
	    while (pp != nullptr && pp->next!=nullptr) {
		    while (i!=j) {
			    if((p->price == pp->next->price && p->time == pp->next->time)||   // if duplicate
          (p->price == pp->next->price && p->time < pp->next->time)||       // if prices are equal, and current time is less than the next. DOMINATION
          (p->price < pp->next->price && p->time == pp->next->time)||       // if current price is cheaper but has identical time as another. DOMINATION
          (p->price < pp->next->price && p->time < pp->next->time))         // if current price is cheaper and its time is also faster. DOMINATION
          return false; 
    ...
    */
    bool is_pareto_sorted() const{
      ...
      // loops if there are multiple nodes in the list
      while (p != nullptr && p->next != nullptr) {
        if ((p->price > p->next->price) || (p->time < p->next->time)) return false;   
    ...
    bool insert_sorted(double price, double time) {
      ...
      Node *pprev = new Node(0.0, 0.0, nullptr);    // node for saving previous node
      Node *ptail = new Node(0.0, 0.0, nullptr);    // node for saving p->next node, used when a value is inserted between two nodes

      p = front;
      insert = new Node(price, time, nullptr);      // the new node to be inserted

      if(!is_sorted()) return false;

      // if list is empty, insert is the first node
      if (front == nullptr) {
        front = insert;
        _size++;
        return true;
      ...
      while (p != nullptr) {
        ...
        if (insert->price < p->price) {
          // if the pprev pointer is not pointing at nowhere
        ...
        if ((insert->price == p->price) &&
            (insert->time >= p->time)) {
              ...
              p->next = insert;   // current node now points to new node
              insert->next = ptail;   // new node points to the saved p->next
              return true;
            }
        else if ((insert->price == p->price) &&
            (insert->time < p->time)) {
              if (pprev->next != nullptr) {
                pprev->next = insert;
                insert->next = p;
    ...
    bool insert_pareto_sorted(double price, double time) {
      ...
      Node *pprev = new Node(0.0, 0.0, nullptr);    // node for saving previous node
      Node *insert = new Node(price, time, nullptr);    // the new node to be inserted

      while (p != nullptr) {
        // checks if new option is suboptimal
        if ((insert->price >= p->price) &&
            (insert->time > p->time)) {
              //std::cout<<"\nNew option is discarded ):"<<std::endl;
              return true;
        }
   ...
   */
    TravelOptions * union_pareto_sorted(const TravelOptions &other)const{
 	    ...
 	    if(!is_pareto_sorted() || !other.is_pareto_sorted()) return nullptr;

      Node *p = front;
      Node *pother = other.front;

      // get the end node of the first list
      while (p->next != nullptr) {
        p = p->next;
      }
      ...
      // I wasn't able to finish the rest of this code, but basically what I had planned to do was 
      // merge the two lists and the perform a sort and prune just like the warmup exercise.
   ...
   */
    bool prune_sorted(){
      ...
      Node *pprev = front;    // node for saving previous node    
      Node *pnext = front->next;     // node for saving next node
      Node *ptail = new Node(0.0, 0.0, nullptr);    // node for saving p->next node, used when a value is inserted between two nodes
      
      while(pnext != nullptr) {
	      ptail = pnext->next;

	      if(pprev->time <= pnext->time){
		      pprev->next = ptail;
          ptail = pnext->next;   
          delete pnext;
	      }
  ...
  TravelOptions * join_plus_plus(const TravelOptions &other) const {
    ...
    // assuming the two lists are the same size based on the warmup
    while (p != nullptr) {
      pnew = new Node(p->price + pother->price, p->time + pother->time, pnew);  // make a new node for join plus plus operation

      pother = pother->next;
      p = p->next;
    }

    Node *newList = pnew->next;

    // I was not able to finish this function, but basically what I tried to do was that assuming the two
    // lists are similar to that of the warmup (same size), I will add the two members together and then make a 
    // new list from that. I was having a hard time trying figure out how to return a TravelOptions object.
    return (TravelOptions*)newList;  // placeholder to make the compiler happy
   ...
   */
   TravelOptions * join_plus_max(const TravelOptions &other) const {
    ...
    // assuming the two lists are the same size based on the warmup
    while (p != nullptr) {
      if (p->time > pother->time) maxTime = p->time;
      else if (p->time < pother->time) maxTime = pother->time;
      else maxTime = p->time;     // does not matter which time to store since they are equal

      pnew = new Node(p->price + pother->price, maxTime, pnew);  // make a new node for join plus max operation
    ...
    Node *newList = pnew->next;
    // Like join_plus_plus, I was not able to finish this code also. Figuring out how to return a TravelOptions object 
    // was my main problem.
   	return (TravelOptions*)newList;
   }
  ...
  TravelOptions * split_sorted_pareto(double max_price) {
	  ...
	  while (p != nullptr) {
		  if (p->price == max_price) {
			  pnxt = p->next;     // save p->next
			  p->next = nullptr;  
			  return (TravelOptions*) pnxt;
		  }
		p=p->next;
	  }
